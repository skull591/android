...
public class JacocoInstrumentation extends Instrumentation implements FinishListener {
    ...
    public static String sTAG = "JacocoInstrumentation:";
    @SuppressWarnings("checkstyle:StaticVariableName") private static String DEFAULT_COVERAGE_FILE_PATH;
    ...
    private void generateCoverageReport() {
        Log.d(sTAG, "generateCoverageReport():" + getCoverageFilePath());
...
//            reset - if true the current execution data is cleared afterwards
    ...
    public void dumpIntermediateCoverage(String filePath) {
        if (LOGD) {
            Log.d(sTAG, "Intermediate Dump Called with file name :" + filePath);
        }
        if (mCoverage) {
            if (!setCoverageFilePath(filePath)) {
                if (LOGD) {
                    Log.d(sTAG, "Unable to set the given file path:" + filePath + " as dump target.");
                }
            }
